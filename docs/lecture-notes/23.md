...
前面所有章节加起来，已经能完成一次正确的更新——但“正确”和“高效”是两回事：现在每次 `setState`，都会从根节点开始，把整棵组件树重新执行一遍，哪怕绝大多数组件的输入根本没变。本章要解决的问题只有一个：**React 凭什么证据，才能安全地断定“这个组件、这棵子树，这一轮什么都不用做”，从而放心跳过它？** 证据不够就跳过，会导致界面显示过期数据；证据太保守，优化就形同虚设。本章会把前面 22 章建立的 Fiber、Lane、Hooks、Context 全部串起来，专门回答这一个问题。
...
这四项只回答“组件输出是否可能变化”，还不是 `beginWork` 的完整决策过程——实际判断要先按 update/mount 分流，并且要在两个不同时机各检查一次 bailout。下面这张图提前给出整体轮廓，具体每一步为什么这样设计，留到 23-4 逐段展开：

```mermaid
flowchart TD
    A["beginWork"] --> B{"update / mount？"}
    B -->|"update"| C{"是否命中 bailout 策略？"}
    B -->|"mount"| F["根据 wip.tag 区别对待"]
    C -->|"否"| F
    C -->|"是"| G{"优化程度"}

    F --> D{"是否命中 bailout 策略？"}
    F --> H["mountChildFibers"]

    D -->|"是"| G
    D -->|"否"| E["reconcileChildFibers"]

    G -->|"高"| I["跳过子树 beginWork"]
    G -->|"低"| J["复用的子 fiberNode"]

    subgraph RECONCILE["reconcile 流程"]
        E --> K["带 flags 的子 fiberNode"]
        H --> L["子 fiberNode"]
    end
```
...
- **bailout 判断出现了两次**：update 分支进入 `beginWork` 时先判断一次（对应四要素里 props/context 引用没变、Fiber 自身没有待处理 lane 这类“执行前就能确认”的情况）；`根据 wip.tag 区别对待`（也就是真正执行组件函数）之后还会再判断一次（对应“组件执行完才发现 state 计算结果没变”的情况）。两次命中的路径最终都汇合到同一个“优化程度”判断，但触发时机不同——这正是 23-4 要讲的“两级 bailout”。
- **“优化程度”决定的是跳过多大范围**：命中 bailout 之后还要再看后代有没有工作——`高`对应“连后代都没有工作，直接跳过整棵子树的 beginWork”；`低`对应“当前节点可以复用，但后代还有工作，只能复用当前这一层，继续往下”。
- **mount 分支完全不经过第一次 bailout 判断**：mount 时没有 current 可以比较，直接进入 `根据 wip.tag 区别对待` 和 `mountChildFibers`，第二次 bailout 判断也不会命中——bailout 本质上是“复用已有结果”，mount 阶段没有“已有结果”可复用。
