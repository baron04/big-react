...
React 能从“这是一次 click”猜出一个默认优先级，但猜不出业务语义——同一次操作里，输入框的值必须立刻更新，但根据这个值算出来的一个巨大列表，明明可以稍微晚一点再显示。这两次调用在 React 眼里都只是普通的 `setState`，没有任何区别。本章要解决的是：怎么让开发者自己声明“这次更新不急”，同时保证被推迟的那部分更新不会丢、不会乱序，最终还是会在合适的时机补上。
