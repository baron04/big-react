...
前面几章都靠“改代码、开浏览器、肉眼看结果”来验证——这在页面简单时够用，但没法验证“数字 key 有没有被转成字符串”这种看不见、却会在后面章节埋雷的中间状态。本章要解决的是：怎么把 ReactElement 的这些内部约定，变成一份可以反复跑、跑坏了立刻报错的证据，而不是每次改动都要重新用眼睛看一遍。
