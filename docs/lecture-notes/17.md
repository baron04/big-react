...
同步渲染有个死结：一旦一次 render 的计算量很大，主线程就会被连续占用十几毫秒甚至更久，浏览器在这段时间里没法响应用户的点击或滚动。把整个更新塞进微任务或 `setTimeout` 都不能真正解决问题——只是换了个时间点继续独占主线程。本章不改 reconciler 一行代码，先用一个和 React 无关的小 Demo 想清楚：怎么把一段长任务拆成一小段一小段，中间主动检查“要不要把主线程还给浏览器”，事后还能从中断的地方接着算。这套机制想明白了，第 18 章才会把它真正接到 Fiber 上。
