...
“ref”这个词在日常使用里指的好像是一件事，但源码里其实是两套完全不相关的机制,只是刚好共用一个名字。`useRef(0)` 返回的那个 `{ current }` 对象，跟 `<div ref={ref}>` 里那个把真实 DOM 节点交给你的过程，走的是两条完全独立的链路——一条挂在 Hook 上、只在 render 阶段起作用；另一条挂在 Fiber 上、要等到 commit 阶段 DOM 真正落地之后才发生。本章要把这两条链路分开讲清楚，也会顺带指出：只有先弄清楚它们为什么互不依赖，才能看懂本章代码里“替换 ref”这个操作实际上还没有真正接通。
