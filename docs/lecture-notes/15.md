...
render 函数理应是纯计算——给定 props/state，算出 UI，不应该有副作用。但订阅、网络请求这些事总得在某处发生。如果直接写在组件函数体里，问题是：render 可能被算了却从没真正提交给用户看，也可能被同一份输入重复调用——这两种情况都不该触发一次真正的订阅。本章要解决的是：怎么让“要做的这件事”和“UI 有没有真的落地”绑在一起——只有这次计算结果真正提交之后，才允许执行；旧的资源也要在新的执行之前正确清理掉。
...
| layout effect    | 未实现 `useLayoutEffect`                                                          | 有 layout effect，在 DOM mutation 后、绘制前同步执行                                        |
| 隐藏树           | 本章不处理；第 22 章加入简化 Offscreen 显隐，但没有补齐 passive effect 挂起语义   | 对 Offscreen/隐藏树会挂起 passive effects，直到可见                                         |
