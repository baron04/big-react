...
组件经常需要返回好几个平级的子节点，但又不想为此多包一层没有意义的 `<div>`——多出来的这层可能打乱 CSS 布局，也可能破坏语义化的 HTML 结构。最省事的做法是让编译器直接把 `<>...</>` 展开、假装它不存在，但这样一来，“这一组节点算作同一个身份”这件事就没地方记录了，第12章刚建立的 key 匹配机制会失效。本章要解决的是：怎样让这一组节点在 Fiber 树里保留身份和边界，却完全不在真实 DOM 里留下痕迹。
...
| `React.Fragment` 导出 | 只有 jsx-runtime 两个入口导出；index.ts 未导出，课程正文不支持 `React.Fragment`            | 两个 runtime 入口与 `React.Fragment` 同时可用  |
