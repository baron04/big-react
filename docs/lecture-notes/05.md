...
一棵 Fiber 树要靠深度优先遍历来处理，但“深度优先遍历”这个词掩盖了一个麻烦：往下走的时候（父节点告诉子节点该长什么样）和往上走的时候（子节点都做完了、父节点该组装结果了），要读的信息完全不同，做的事情也完全相反。如果把这两件事塞进同一个函数，父节点要么在子节点还没做完时就仓促创建 DOM，要么得自己再递归一遍——工作边界又回到了第3章想摆脱的那个不可控调用栈。本章要做的，就是把这一次遍历拆成方向相反的两半：`beginWork`（递，父到子）和 `completeWork`（归，子到父）。
...
- **本章实现边界**：本章只定义 HostRoot、HostComponent、HostText 三类 Fiber 各自的 begin/complete 规则，`workLoop` 仍是单次 `if`，一次调用只推进一个工作单元，还不能走完整棵树；DOM hostConfig 也没有具体实现。下文不再逐句重复这条边界。
...
这相当于把递归函数调用前的逻辑放进 begin，把递归返回后的逻辑放进 complete。具体到每一步在做什么、写了什么：

```text
begin section  -> 读 section 的 children，创建 span 的 Fiber，span.return = section，返回 span
begin span     -> 读 span 的 children，创建文本 Fiber，text.return = span，返回 text
begin text(A)  -> 文本节点没有 children，返回 null
complete text(A) -> 创建文本节点的宿主实例（真实文本节点），挂到 text.stateNode
complete span  -> 把 text 的宿主实例组装进 span 的宿主实例，挂到 span.stateNode
complete section -> 把 span 的宿主实例组装进 section 的宿主实例，挂到 section.stateNode
```

**begin 阶段一路往下创建 Fiber 和 `return` 指针，但完全不创建宿主实例；宿主实例全部在 complete 阶段、从叶子往根方向组装出来。** 这也是为什么最外层的 `section` 反而是最后一个拿到宿主实例的节点——它要等 `span`（进而 `text`）都完成了，才能把已经组装好的子实例塞进自己。上面是指针算法完整运行后的顺序；本章代码一次只执行一个工作单元，第 6 章把 `if` 改成 `while` 后才会在一次 render 中走完它。[performUnitOfWork](../../packages/react-reconciler/src/workLoop.ts)负责选择下一指针：有 child 就向下，没有 child 就完成当前并沿 return 回溯，遇到 sibling 再转向同层：
...
mount 没有旧树可比较，但仍要把 children 输入转成 Fiber。**“reconcile”这个词容易被误解成“专门处理更新时的比较”，但它的真正职责更宽——是把这次声明输入规范化成下一棵 Fiber 子树，“比较旧节点”只是这个过程在有旧树可比较时会做的事。** 本章的 `childFibers.ts` 只识别一个 ReactElement，或一个 string/number 文本，并写好新 Fiber 的 `return`；数组、Fragment 与多兄弟节点尚未支持。mount 没有旧节点可比较，但“把输入转成 Fiber”这一步一次都不能少。
...
beginWork 向下递归，产出下一层 Fiber，不是 DOM；completeWork 向上回溯，产出离屏宿主实例和 flags 摘要，不是页面变化。整章的两节分别对应这一递一归：5-1 实现 beginWork 怎样根据输入产出 child，5-2 实现 completeWork 怎样组装宿主实例并冒泡 flags。需要特别提醒：本章 `workLoop` 每次只调用一次 `performUnitOfWork`，所以下面写出的这些单元规则，本章还不能在一次调度中连续走完整棵树——这是第 6 章把 `if` 换成 `while` 之后才会发生的事。
...
这正是第 4 章留下的那个悬念第一次被接上：第 4 章的 `Update` 一直只是静静地待在 `shared.pending` 里，`HostRoot.memoizedState` 始终没变过；本章 `updateHostRoot` 第一次真正调用 `processUpdateQueue` 去消费它，`memoizedState` 才第一次被写成“当前 root 要渲染的 ReactElement”。
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| 属性处理 | mount 时通过 hostConfig 创建实例；第 10 章接入 update 标记链路，但课程正文未实现 className/style 等普通 DOM 属性更新            | 有完整属性 diff 与事件 props 校验                |
| 遍历方式 | 本章 `workLoop` 还是单次 `if` 起步形态；`while` 循环第 6 章到位，第 18 章改名 `workLoopSync` 并接入并发                         | 同步/并发 work loop 并存                         |
| 支持类型 | 本章只有 HostRoot/HostComponent/HostText；第 7 章加入 FunctionComponent，第 13 章加入 Fragment，第 22 章加入 Suspense/Offscreen | 还有 ClassComponent、Portal、Profiler 等更多 tag |
