...
`onClick` 写在 JSX 里，看起来就是普通的一个 prop，但浏览器完全不认识“prop”这个概念——它只认识 `addEventListener`。最直白的做法是每个节点各自注册一个原生监听器，但这样一来 React 就没法统一控制“点击到底该先执行哪个回调、后执行哪个回调”，也没法在所有事件外面统一包一层批处理和优先级逻辑。本章要解决的就是：怎么只在最外层的 root 注册一次监听，就能让每个节点上声明的 `onClick` 都在正确的时机被正确的顺序调用。
...
| -------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| 委托位置       | 只支持 click，且每次 `root.render` 都会重复注册 listener                          | 在 root container 注册全部「all native events」，并对注册去重                                          |
| 合成事件       | 直接复用原生 Event，只改写 `stopPropagation` 并增加停止标记                       | 用事件插件（SyntheticMouseEvent 等）规范化 `currentTarget`、`preventDefault`、`nativeEvent` 等大量字段 |
| 收集路径       | 沿 DOM `parentNode` 逐级读 `__props`                                              | 维护独立的 fiber -> DOM 映射，用 `getClosestInstanceFromNode` 从 target 找 fiber 再向上收集            |
| 优先级入口     | 本章无任何优先级代码，`eventTypeToSchedulePriority` 与 Scheduler 包裹第 18 章加入 | 完整接入 Lane 体系与连续/离散事件分类                                                                  |
| props 缓存时机 | update 的 completeWork 直接写共享 DOM；第 18 章并发让出后可能暴露未提交 handler   | 在提交相关路径更新节点 props 缓存，事件读取与 current 树保持一致                                       |
