...
一个深层组件要读取的数据还没准备好，这件事该怎么“报告”给页面？最笨的办法是让每一层组件自己判断“数据来了没有、没来就返回一个 loading”，但这样加载状态会散落在整棵树的各个角落，没有一个地方能统一决定“到底该展示什么替代内容”。本章要解决的是：怎么让最深处的一次数据读取，能够直接终止当前这条渲染路径，把控制权交给外层最近的 `<Suspense>` 边界去展示替代 UI，并且在数据真正就绪之后，还能准确地回到原来的状态重新渲染一次——这几乎是全书最难的一章，需要用到异常控制流、Fiber 回溯和 Lane 重试三种机制配合完成。
