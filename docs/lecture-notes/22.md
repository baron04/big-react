...
一次 render 必须产出一棵完整、可提交的候选树。但如果深层组件通过 `use` 读取的 thenable 仍处于 pending，它就无法完成本次 render。

难点不只是“先显示一段 loading”，而是同时满足三个约束：

1. **发现挂起与处理挂起不在同一位置**：发现数据未就绪的是深层组件，决定展示什么的是最近的 `<Suspense>` 祖先，中间组件不应该逐层传递特殊返回值。
2. **失败路径不能产生半成品 UI**：primary 没有完成，就不能提交已经计算出的局部结果；React 必须回到边界，改为构造一棵完整的 fallback 树。
3. **数据就绪后只能重试，不能续接**：thenable settle 时，原来的 JavaScript 调用栈已经退出，渲染期间的输入也可能变化，因此 React 必须重新发起 render，再次验证 primary 能否完成。

所以 Suspense 不只是“显示 Loading”的组件，而是一套 render 控制流：

```text
深层组件挂起
-> 最近的 Suspense 接管
-> 回退失败路径并渲染 fallback
-> commit 完整的 fallback UI
-> thenable settle 后重新调度
-> 新 render 再次尝试 primary
```

本章要实现的，就是这条从挂起、接管到重试的闭环。
