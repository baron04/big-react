...
Context 常被当成“一个方便读取的全局变量”，但它其实要解决一个更具体的问题：同一个 Context，嵌套的 Provider 可以在树的不同分支给出不同的值，内层结束之后还必须准确恢复外层的值，一点都不能漏。如果只用一个普通变量存“当前值”，根本没法表达“进入这个子树用这个值、离开之后要换回来”这种依赖树形结构的规则。本章要解决的是：怎么让 Context 的读取值，随着 Fiber 树的遍历自动进入和退出正确的作用域。
