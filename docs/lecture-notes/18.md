...
第17章证明了“时间切片”这个机制本身能成立，但那只是一个和 React 完全无关的 Demo。真要把它接到 reconciler 上，会立刻撞上一个更麻烦的问题：暂停一棵正在计算的 Fiber 树，跟暂停一个简单计数器完全不是一回事——如果这时候又有一条优先级更高的更新插进来，之前没算完的那些 state 变化怎么办？不能丢，也不能算错顺序。本章要解决的正是这件事：把优先级从“用户点了一下”这个瞬间，一路准确地带到“这个 Hook 的 state 到底该等于什么”这个终点，中途无论被打断多少次都不出错。
...
Scheduler priority 只能决定“哪个任务先获得主线程”，不能替 reconciler 回答“Hook 队列里的哪些 Update 可以在本轮参与状态计算”。因此 React 会把事件上下文中的临时优先级转换为 Lane，再把 Lane 分别记录到 Update、root 和当前 render 的稳定对象或变量上。

先不要把这些字段看成相似的“优先级缓存”，而应先问每个字段解决哪个层次的问题：

| 对象或字段                       | 它回答的问题                                          | 写入者 → 消费者                                       |
| -------------------------------- | ----------------------------------------------------- | ----------------------------------------------------- |
| `Update.lane`                    | 这条 Update 属于哪种优先级，本轮是否应该计算？        | dispatch → `processUpdateQueue`                       |
| `root.pendingLanes`              | 整个 root 还有哪些优先级的工作没有完成？              | `markRootUpdated` → `ensureRootIsScheduled`           |
| `root.callbackPriority`          | 当前已注册 callback 代表哪个最高优先级，能否复用？    | `ensureRootIsScheduled` → 下一次 root 调度            |
| `root.callbackNode`              | 当前哪个 Scheduler 任务拥有调度权，能否取消或续跑？   | `ensureRootIsScheduled` → 取消任务、continuation 校验 |
| `wipRootRenderLane` / renderLane | 本轮 render 正在处理哪种优先级，允许消费哪些 Update？ | `prepareFreshStack` → beginWork、Hook 队列计算        |
| `Hook.baseState/baseQueue`       | 被跳过的 Update 将来从什么状态、按什么顺序重放？      | `processUpdateQueue` → 下一轮 Hook 状态计算           |

这张表体现了本章的主线：root 根据聚合的 Lane 选择“先做哪批工作”，状态队列再根据单条 Update 的 Lane 判断“这批工作具体包含哪些更新”。每一层只做自己的决策，同时把下一层需要的信息写到可恢复的对象上。
