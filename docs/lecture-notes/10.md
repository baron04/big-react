...
到目前为止的代码只会做一件事：把一棵全新的树创建出来。可现实中的更新是“页面已经有一棵树了，只是某个数据变了”——这时候如果还是当成全新的树处理，会发生什么？本章开头会先带你亲手验证一次这个 bug（新旧内容同时出现在页面上），再解决它：怎么判断“这还是原来那个节点，只是内容变了”，而不是每次更新都整体推倒重来。
...
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| 单节点复用         | `reconcileSingleElement` 按 key/type 判断，复用 `useFiber`                                                                          | `reconcileSingleElement` 思路一致；但会额外处理 lazy、portal 等元素类型                 |
| HostComponent 更新 | 本章 update 分支还是空占位，props 变化不产生宿主操作；第 11 章只加入事件 props 缓存，课程正文未实现普通 DOM 属性更新                | 在 `completeWork` 对 props 做 diff（`diffProperties`），只把变化属性放进 `updatePayload` |
| 文本更新           | `oldText !== newText` 才标 `Update`                                                                                                 | 同样比较，行为一致                                                                       |
| 删除子树           | `commitDeletion` 递归收集最外层 Host 节点并 `removeChild`；第 15 章补 effect destroy，第 20 章补 ref detach，第 22 章加入 Offscreen | 还覆盖更多隐藏树与错误处理分支                                                           |
