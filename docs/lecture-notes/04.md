...
调用 `root.render(element)`，这个 `element` 是怎么从“用户传进来的一个对象”变成“React 真的开始去处理”的？最直接的想法是：拿到 element 就立刻改 HostRoot 的状态、立刻开始 render——本章会说明这个想法为什么行不通，并建立正确的做法：先把“要做什么改动”包装成一个 `Update` 对象存起来，再从触发更新的 Fiber 一路找到根节点，才真正开始计算。
...
- **本章实现边界**：本章只接入 HostRoot 这一条更新来源；深层组件、Hook dispatch 第 8 章才加入。队列本章只有单个 `pending` 槽，第 14 章才改成环形队列；调度、优先级本章完全没有，第 14 章接入 Lane。下文不再逐句重复这条边界。
...
**这里的核心转变不是“要不要更新”，而是“什么时候算”**：不是拿到 element 就立刻算出新状态、立刻改，而是先把“我想怎么变”原样记下来，找到该由谁负责处理之后，再统一去算。有点像点餐——服务员不会你一说“要一份炒饭”就立刻冲进厨房代替厨师炒，而是先把这张单子记下来，交给后厨在合适的时候统一处理。因此 React 把“更新意图”和“执行更新”分开：
...
`n => n + 1` 这个函数在创建 Update 的那一刻，根本不知道 `n` 会是多少——它依赖的是消费时刻的 `baseState`。如果触发时就立刻求值，要么强行提前读一次当前状态（读到的可能不是最终基准），要么根本没法处理函数式更新。所以 action 必须原样存起来，延迟到 `processUpdateQueue` 执行的那一刻才求值；本章单槽 pending 不能同时保存两条 action，不能用它推导连续更新语义——这是第 14 章环形队列要解决的问题。
...
阅读本章源码时不要把 enqueue、找 root 和 render 混成一个笼统的“setState”。它们分别保存内容、解析执行范围和启动计算——这三件事本章由三组不同的函数分别负责，第 14、18 章只会扩展队列与调度策略，不会改变这条职责链：
...
本章只有 `action` 和单个 `shared.pending`。它能证明“先把意图放进所属 Fiber”，但还不能证明 render 已经消费它；连续入队还会覆盖前一条记录——这个限制会在下面反复出现，第 14 章才把 pending 改为环形队列。
...
以 `updateContainer(element, root)` 为例，走一遍完整链路：
...
element 没有直接传给 work loop，而是先包装成 Update。当前收益只是触发处与状态计算函数解耦；由于 beginWork 尚未调用 `processUpdateQueue`，`shared.pending` 仍保留这条 Update，`HostRoot.memoizedState` 也没有变化。（提前说明一个后面会用到的心智模型：HostRoot 的 `memoizedState` 就是“当前 root 要渲染的 ReactElement”——第 5 章消费队列后，写入的正是这个字段。）

独立调用 `processUpdateQueue` 时，值 action 会直接成为结果，函数 action 会接收传入的 `baseState`——这验证的是它的纯函数行为本身：给定输入，能推出确定输出。但本章 render 没有调用这个函数，所以不能声称页面或 HostRoot 状态已经更新。这也是本章反复要区分的两件事：**Update 被正确保存到了 queue 里，和 Update 已经被消费、状态已经改变，是两个完全不同的断言**——前者本章成立，后者要等第 5 章 `beginWork` 真正调用 `processUpdateQueue` 才成立。

这也是本章的队列边界所在：单槽 `pending` 只为验证“Update 属于哪个 queue、queue 属于哪个 Fiber、render 从哪里消费”这条最小链路，不能保存两次 render 之间连续到达的多条 Update，因此不能据此推导完整批处理语义。`scheduleUpdateOnFiber` 目前也不处理找不到 root 的情况；big-react 项目里通常会在开发环境告警并返回。第 14 章会在调度允许更新积累时，把该字段改成环形队列。
...
| 调度     | big-react 到第 14 章才引入 Lane 与微任务/Scheduler                          | 有完整调度模型         |
| 批处理   | 本章单槽 pending 会丢连续更新，批处理要到第 14 章环形链表与调度接入后才成立 | 自动批处理覆盖更多场景           |
