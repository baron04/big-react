...
describe('Suspense test', () => {
	...
	test('known deviation: switching thenable shows previous primary alongside fallback', async () => {
		const cache = new Map();
		...
		// 已知偏差：官方 React 会把旧 primary 隐藏（Offscreen）只留 fallback，本实现两者同时可见。
		// 若将来对齐官方实现，应同步修改这条断言，而不是当作回归。
		expect(root).toMatchRenderedOutput(
			<>
				<div>Child:1</div>
				{'Loading'}
